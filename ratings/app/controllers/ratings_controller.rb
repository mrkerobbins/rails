...
# Since ratings are shown by professor, it does not make sense
# to show a single rating.
...
class RatingsController < ApplicationController
  ...
  # that way the Rating's foreign key (professor_id) will
  # be initialized correctly.
  def new
    # since our new rating path contains the professor's id
    # we can use params[:professor_id] to get the id.
  ...
  # Rails ensures that the URL has not been tampered with.
  def create
    @professor = Professor.find params[:professor_id]
